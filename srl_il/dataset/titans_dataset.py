...
class TitansTrajectorySequenceDataset(TrajectoryDataset):
    """
            ...
            with h5py.File(f, "r", swmr=True, libver='latest') as h5_file:
                ...
                try:
                    ...
                    for img_name, img_data in obs_group.get("images", {}).items():
                        traj_data[f"{
                            img_name}/color"] = (img_data["color"], f, img_data["color"].name)
                        if "extrinsics" in img_data:
                            global_data[f"{
                                img_name}/extrinsics"] = (img_data["extrinsics"], f, img_data["extrinsics"].name)
                        if "intrinsics" in img_data:
                            global_data[f"{
                                img_name}/intrinsics"] = (img_data["intrinsics"], f, img_data["intrinsics"].name)
                        if "projection" in img_data:
                            global_data[f"{
                                img_name}/projection"] = (img_data["projection"], f, img_data["projection"].name)
                ...
                if self.traj_data is None:
                    ...
                    for key in list(self.traj_data.keys()):
                        if key in traj_data:
                            self.traj_data[key].append(
                                self._write_cache_or_not(traj_data[key]))
                        else:
                            self.traj_data.pop(key)
                            print(f"Warning: key {key} not found in {f}")
                    for key in list(self.global_data.keys()):
                        if key in global_data:
                            self.global_data[key].append(
                                self._write_cache_or_not(global_data[key]))
                        else:
    ...
    def _write_cache_or_not(self, data_and_name):
        ...
        if data.size < self.cache_data_size:
            ...
            # return the path to the h5 file and the data name
    ...
    def _read_cache_or_not(self, data_or_name):
        if type(data_or_name) == tuple:
            ...
            if file_name not in self._h5_files:
                self._h5_files[file_name] = h5py.File(
                    file_name, "r", swmr=True, libver='latest')
            # read the reference from h5
            return self._h5_files[file_name][data_name]
        else:
